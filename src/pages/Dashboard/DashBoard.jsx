...
function DashBoard() {
  ...
  useEffect(() => {
    ...
    console.log("nextStep from storage:", nextStep);
    if (nextStep) {
      const steps = currentEvent(nextStep);
      console.log("calculated steps:", steps);
      setShowSteps(steps);
    } else {
      // If no next_step is stored, default to first step (PAN Authentication)
      console.log("No next_step found, defaulting to 0");
      setShowSteps(0);
    }
  }, []);
